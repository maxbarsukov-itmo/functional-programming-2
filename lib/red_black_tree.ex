defmodule RedBlackTree do
  ...
  def reduce_left(tree, acc, fun) do
    to_list(tree) |> fold_left(acc, fun)
  end
  ...
  @spec get_and_update(RedBlackTree.t(), any(), (any() -> any())) :: {any(), RedBlackTree.t()}
  ...
  def equal_keys?(%RedBlackTree{} = tree1, %RedBlackTree{} = tree2) do
    reduce(tree1, true, fn {key1, _}, acc ->
      acc && has_key?(tree2, key1)
    end) &&
      reduce(tree2, true, fn {key2, _}, acc ->
        acc && has_key?(tree1, key2)
      end)
  end
  ...
  def to_list(%RedBlackTree{root: nil}), do: []
  ...
  # again to the slower phash function distributed over @key_hash_bucket integers.
  ...
  defp do_reduce_nodes(_order, nil, acc, _fun), do: acc
...
defimpl Enumerable, for: RedBlackTree do
  ...
  def count(%RedBlackTree{size: size}), do: {:ok, size}
